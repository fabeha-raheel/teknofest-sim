...
xy_low_noise_std = 2
xy_medium_noise_std = 5
xy_high_noise_std = 10

z_low_noise_std = 0.005
...
def Water_Discharge_Detected_Callback_function(data_recieve):
    ...
    Z_position= current_pose.pose.position.z + gaussian_noise(z_current_noise_std)
    X_Error= setPointX-currentX + gaussian_noise(xy_current_noise_std)       #Remember these errors are caluclated in the image frame of reference where
    Y_Error= setPointY-currentY + gaussian_noise(xy_current_noise_std)       #(x,y) is the top left corner
    ...
    if (lap_counter >1 and Water_Discharge_Location_Detected_Lap_02 and not(EXECUTING_WAYPOINT_NAVIGATION)):
        ...
        if distance_to_center > THRESHOLD_FOR_DISTANCE_TO_CENTER:
            ...
            twist.twist.linear.y=0.1*controller_x.set_current_error(-X_Error)
            twist.twist.linear.x=0.1*controller_y.set_current_error(-Y_Error)
